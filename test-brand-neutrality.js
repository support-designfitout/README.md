...
// Test dual domain configuration
function testDualDomainConfig() {
  console.log('Testing dual domain configuration...');
  
  try {
    const domainConfigPath = path.join(__dirname, 'public/domain-config.js');
    if (!fs.existsSync(domainConfigPath)) {
      throw new Error('domain-config.js not found in public folder');
    }
    
    const domainConfigContent = fs.readFileSync(domainConfigPath, 'utf8');
    
    // Check for required domain configuration elements
    ...
    for (const element of requiredElements) {
      if (!domainConfigContent.includes(element)) {
        throw new Error(`Missing required element in domain-config.js: ${element}`);
      }
    }
    
    // Verify HTML files include domain-config.js
    const htmlFiles = ['public/index.html', 'public/ind2x.html'];
    for (const htmlFile of htmlFiles) {
      const htmlPath = path.join(__dirname, htmlFile);
      if (fs.existsSync(htmlPath)) {
        const htmlContent = fs.readFileSync(htmlPath, 'utf8');
        if (!htmlContent.includes('domain-config.js')) {
          throw new Error(`${htmlFile} does not include domain-config.js`);
        }
      }
    }
    
    console.log('✅ Dual domain configuration is properly implemented');
    return true;
  } catch (error) {
