...
## 🚀 Smoke E2E Tests (Production)

For production-focused end-to-end testing with strict cache enforcement, see:
- **Documentation**: [`SMOKE_E2E_TESTS.md`](./SMOKE_E2E_TESTS.md)
- **Workflow**: `.github/workflows/smoke-e2e.yml`
- **Test Files**: `tests/{health,perf,redirects}.spec.ts`
- **Environment**: Production (https://app.fitoutlab.app)
- **Cache Policy**: Enforced (`REQUIRE_CACHE_HIT=true`)

Quick start for smoke tests:
```bash
npm run test:e2e-structure  # Validate structure without browsers
export BASE_URL=https://app.fitoutlab.app REQUIRE_CACHE_HIT=true
npm run test                # Run against production
```
