...
// Import Studio36 OS API handler
const { handleStudio36Request } = require('./api/studio36');
...
export default {
  async fetch(request) {
    ...
    // Route to Studio36 OS API endpoints
    if (url.pathname.startsWith('/api/studio36')) {
      return handleStudio36Request(request, url);
    }
    ...
    if (url.pathname === '/health' || url.pathname === '/api/status') {
      return new Response(JSON.stringify({
        ...
        endpoints: {
          ...
          // Studio36 OS endpoints
          studio36: '/api/studio36',
          moodBoards: '/api/studio36/mood-boards',
          samples: '/api/studio36/samples',
          deliveries: '/api/studio36/deliveries',
          vendors: '/api/studio36/vendors',
          contractors: '/api/studio36/contractors',
          bookings: '/api/studio36/bookings',
          campaigns: '/api/studio36/campaigns',
          projects: '/api/studio36/projects'
        }
      }), {
...
exports.handler = async (event, context) => {
  ...
  try {
    ...
    if (path.startsWith('/api/studio36')) {
      const mockRequest = {
        method: event.httpMethod,
        url: `https://example.com${path}`,
        headers: {
      ...
      const response = await handleStudio36Request(mockRequest, mockUrl);
      const data = await response.json();
