...
func TestComputeBilling(t *testing.T) {
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			result, err := ComputeBilling(tt.computation)
			...
			if tt.expectError {
				...
				if tt.errorMsg != "" && err.Error() != "invalid computation: "+tt.errorMsg {
			...
			if result.ProjectID != tt.expected.ProjectID {
				t.Errorf("ProjectID: expected %s, got %s", tt.expected.ProjectID, result.ProjectID)
			}

			if result.BaseAmount != tt.expected.BaseAmount {
				t.Errorf("BaseAmount: expected %f, got %f", tt.expected.BaseAmount, result.BaseAmount)
			}

			if result.DiscountAmount != tt.expected.DiscountAmount {
				t.Errorf("DiscountAmount: expected %f, got %f", tt.expected.DiscountAmount, result.DiscountAmount)
			}

			if result.TaxAmount != tt.expected.TaxAmount {
				t.Errorf("TaxAmount: expected %f, got %f", tt.expected.TaxAmount, result.TaxAmount)
			}

			if result.TotalAmount != tt.expected.TotalAmount {
				t.Errorf("TotalAmount: expected %f, got %f", tt.expected.TotalAmount, result.TotalAmount)
			}

			if result.Currency != tt.expected.Currency {
				t.Errorf("Currency: expected %s, got %s", tt.expected.Currency, result.Currency)
			}

			if result.ComputationID != tt.expected.ComputationID {
				t.Errorf("ComputationID: expected %s, got %s", tt.expected.ComputationID, result.ComputationID)
			}

			// Verify breakdown is present for successful computations
			if len(result.Breakdown) == 0 {
				t.Errorf("Expected breakdown but got empty slice")
			}
		})
	}
}
...
func TestValidateDiscount(t *testing.T) {
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			err := validateDiscount(tt.discount)
