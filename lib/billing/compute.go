...
// BillingLine represents a line item in the billing breakdown
...
type BillingResult struct {
	...
	ComputedAt     time.Time     `json:"computed_at"`
	Breakdown      []BillingLine `json:"breakdown,omitempty"`
}

// BillingBreakdown provides detailed breakdown of billing computation
// Deprecated: Use BillingResult.Breakdown []BillingLine instead
...
func ComputeBilling(computation BillingComputation) (*BillingResult, error) {
	...
	breakdown := []BillingLine{
		{
			Description: fmt.Sprintf("%s - %.2f hours @ %s %.2f/hour",
				computation.Rate.ServiceType, computation.Hours,
				computation.Rate.Currency, computation.Rate.HourlyRate),
			Amount: roundToTwoDecimals(baseAmount),
		},
	}

	// Add discount lines
	for _, discount := range computation.Discounts {
		discountDesc := fmt.Sprintf("Discount (%s)", discount.Type)
		if discount.Reason != "" {
			discountDesc = fmt.Sprintf("Discount: %s (%s)", discount.Reason, discount.Type)
		}

		var discountLineAmount float64
		if discount.Type == "percentage" {
			discountLineAmount = -roundToTwoDecimals(baseAmount * (discount.Value / 100))
		} else {
			discountLineAmount = -roundToTwoDecimals(discount.Value)
		}

		breakdown = append(breakdown, BillingLine{
	...
	// Add tax lines
	for _, tax := range computation.Taxes {
		taxDesc := fmt.Sprintf("Tax (%s)", tax.Type)
		if tax.Name != "" {
			taxDesc = fmt.Sprintf("Tax: %s (%s)", tax.Name, tax.Type)
		}
		if tax.Region != "" {
			taxDesc = fmt.Sprintf("%s [%s]", taxDesc, tax.Region)
		}

		var taxLineAmount float64
		if tax.Type == "percentage" {
			taxLineAmount = roundToTwoDecimals(amountAfterDiscount * (tax.Value / 100))
		} else {
			taxLineAmount = roundToTwoDecimals(tax.Value)
		}
