...
class StrategicAnalysis {
    ...
    generateInsights(competitorData) {
        const avgScores = this.calculateAverageScores(competitorData);
        const ourAdvantage = this.calculateAdvantage(avgScores);
        
        return {
            competitorAverage: avgScores,
            ourAdvantage: ourAdvantage,
            recommendations: this.generateRecommendations(ourAdvantage),
            marketPosition: this.calculateMarketPosition(avgScores),
            timestamp: new Date(),
...
if (window.ModuleSystem) {
    window.ModuleSystem.register('strategicAnalysis', () => {
        return new Promise((resolve) => {
            const instance = new StrategicAnalysis();
            resolve(instance);
        });
    }, []);
}
